...
def poke_coord(fichier_entree, fichier_sortie, nombre_lignes):
    ...
    # On calcule les poids de rareté en fonction des valeurs de rareté
    poids_rarete = df['Rare'].apply(lambda x: x)  

    # On sélectionne aléatoirement des lignes avec remplacement, en tenant compte des poids de rareté
    lignes_aleatoires = df.sample(n=nombre_lignes, replace=True, weights=poids_rarete)

    # On génère les coordonnées uniques pour X et Y
    coordonnees = generer_coordonnees_uniques(nombre_lignes)

    # On ajoute les colonnes X et Y au DataFrame
    lignes_aleatoires['X'] = [x for x, _ in coordonnees]
    lignes_aleatoires['Y'] = [y for _, y in coordonnees]

    # On écrit les lignes dans un nouveau fichier CSV
    # On enregistre le DataFrame modifié dans le fichier CSV d'origine
